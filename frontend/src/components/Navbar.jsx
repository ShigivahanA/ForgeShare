...
import { Hammer, X, ArrowUpRight, Globe, MessageCircle, Camera, Mail, MapPin, Phone, LogOut, User as UserIcon } from 'lucide-react'
...
export default function Navbar() {
  ...
  const MENU_LINKS = [
    { id: '01', title: 'How It Works', href: '/how-it-works' },
    { id: '02', title: 'Browse Tools', href: '/rent' },
    ...(user ? [
      ...(user.role === 'admin'
        ? [{ id: '03', title: 'Guild Users', href: '/admin/users' }]
        : [{ id: '03', title: 'The Vault', href: '/wishlist' }]
      ),
      ...(user.role === 'admin'
        ? [{ id: '04', title: 'Admin Hub', href: '/admin' }]
        : [{ id: '04', title: 'List Gear', href: '/list' }]
      )
    ] : [])
  ]
  ...
  return (
    <>
      ...
      <motion.header
        initial={{ y: 0 }}
        animate={{ y: (isVisible || isHovered || isOpen) ? 0 : '-100%' }}
        transition={{ duration: 0.4, ease: [0.33, 1, 0.68, 1] }}
        className={`fixed top-0 left-0 w-full z-[100] transition-all duration-300 ${scrollY.get() > 20 && !isOpen ? 'h-16 bg-artisan-dark/90 backdrop-blur-md' : 'h-24'
          }`}
      >
        <div className="container-custom h-full flex items-center justify-between border-b border-artisan-light/5">
          ...
          >
            ...
            >
              <Hammer className="w-5 h-5 text-artisan-light" />
              <span className="text-lg font-display font-bold uppercase tracking-[0.2em]">ForgeShare</span>
            </motion.div>
          </Link>
              ...
              >
                ...
                {!user && (
                  <div className="overflow-hidden lg:hidden">
                    ...
                    >
                      <motion.div variants={linkVariants} className="flex items-baseline gap-4 md:gap-6">
                        <span className="text-[10px] md:text-xs font-mono text-artisan-grey">03</span>
                        <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-8xl font-display font-bold uppercase tracking-tight group-hover:text-outline transition-all duration-500 leading-[1.1]">
                          Join The Guild
                        </h2>
                        <ArrowUpRight className="w-5 h-5 md:w-8 md:h-8 lg:w-10 lg:h-10 text-artisan-light opacity-0 group-hover:opacity-100 -translate-x-4 group-hover:translate-x-0 transition-all duration-500" />
              ...
              <div className="lg:col-span-5 xl:col-span-4 flex flex-col gap-8 md:gap-12 lg:pl-12 lg:border-l border-artisan-light/5">
                ...
                >
                  <p className="text-xs font-mono uppercase tracking-[0.2em] text-artisan-grey">
                    {user
                      ? (user.role === 'admin' ? 'Admin Console' : 'Artisan Portal')
                      : 'Connections'
                    }
                  </p>
                ...
                >
                  <p className="text-xs md:text-sm font-mono text-artisan-light/60 uppercase tracking-[0.2em] leading-relaxed">
                    ForgeShare Network<br />
                    The Artisan Way
                  </p>
                </motion.div>
              </div>
